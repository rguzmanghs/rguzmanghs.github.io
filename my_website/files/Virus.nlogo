turtles-own
  [
    sick1?               ;; if true, the turtle is infectious with disease 1

sick2?               ;; if true, the turtle is infectious with disease 2
    remaining-immunity1  ;; how many weeks of immunity the turtle has left to disease 1
    remaining-immunity2  ;; how many weeks of immunity the turtle has left to disease 2
    sick-time            ;; how long, in weeks, the turtle has been infectious
    age ]                ;; how many weeks old the turtle is

globals
  [ %infected            ;; what % of the population is infectious
    %immune1             ;; what % of the population is immune to disease 1
    %immune2             ;; what % of the population is immune to disease 2
    %immune              ;; what % of the population is immune to either disease 1 or 2
    lifespan            ;; lifespan of turtles
...
to setup-turtles
  create-turtles number-people
    [ setxy random-xcor random-ycor
      ...
      set sick-time 0
      set remaining-immunity1 0
      set remaining-immunity2 0
...
to become-immune1 ;; turtle procedure
  set sick1? false
  set sick-time 0
  set remaining-immunity1 immunity-duration1
end

to become-immune2 ;; turtle procedure
  set sick2? false
  set sick-time 0
  set remaining-immunity2 immunity-duration2
end
...
set lifespan 50 * 52      ;; 50 times 52 weeks = 50 years = 2600 weeks old
  ...
  set immunity-duration1 52
  set immunity-duration2 52
end
...
to go
  ask turtles [
    get-older
    move
    if sick1? [ recover-or-die1 ]
    if sick2? [ recover-or-die2 ]
    ifelse sick1? [ infect1 ] [ reproduce ]
    ifelse sick2? [ infect2 ] [ reproduce ]
  ]
  update-global-variables
  update-display
  tick
...
to update-global-variables
  if count turtles > 0
    [ set %infected ((count turtles with [ sick1? ] + count turtles with [ sick2? ]) / count turtles) * 100
      set %immune (count turtles with [ immune1? ] + count turtles with [ immune2? ] / count turtles) * 100 ]
end

to update-display
  ask turtles
    [ if shape != turtle-shape [ set shape turtle-shape ]
      set color ifelse-value sick1? [ red ] [ifelse-value sick2? [ yellow ] [ifelse-value immune1? [ grey ] [ifelse-value immune2? [ brown ] [ green ] ] ] ] ]
end
...
to get-older ;; turtle procedure
  ...
  set age age + 1
  if age > lifespan [ die ]
  if immune1? [ set remaining-immunity1 remaining-immunity1 - 1 ]
  if immune2? [ set remaining-immunity2 remaining-immunity2 - 1 ]
  if sick1? [ set sick-time sick-time + 1 ]
  if sick2? [ set sick-time sick-time + 1 ]
end
...
to infect1 ;; turtle procedure
  ask other turtles-here with [ not sick1? and not immune1? and not sick2? ]
    [ if random-float 100 < infectiousness1
      [ get-sick1 ] ]
end

to infect2 ;; turtle procedure
  ask other turtles-here with [ not sick2? and not immune2? and not sick1? ]
    [ if random-float 100 < infectiousness2
      [ get-sick2 ] ]
end

;; Once the turtle has been sick long enough, it
;; either recovers (and becomes immune) or it dies.
to recover-or-die1 ;; turtle procedure
  if sick-time > duration1                        ;; If the turtle has survived past the virus' duration, then
    [ ifelse random-float 100 < chance-recover1   ;; either recover or die
      [ become-immune1 ]
      [ die ] ]
end

to recover-or-die2 ;; turtle procedure
  if sick-time > duration2                        ;; If the turtle has survived past the virus' duration, then
    [ ifelse random-float 100 < chance-recover2   ;; either recover or die
      [ become-immune2 ]
      [ die ] ]
end
...
to-report immune1?
  report remaining-immunity1 > 0
end

to-report immune2?
  report remaining-immunity2 > 0
end
...
"" ""
PENS
"sick1" 1.0 0 -2674135 true "" "plot count turtles with [ sick1? ]"
"immune" 1.0 0 -7500403 true "" "plot count turtles with [ immune1? ]"
"healthy" 1.0 0 -10899396 true "" "plot count turtles with [ not sick1? and not sick2? and not immune1? and not immune2? ]"
"total" 1.0 0 -13345367 true "" "plot count turtles"
"sick2" 1.0 0 -1184463 true "" "plot count turtles with [ sick2? ]"
"pen-5" 1.0 0 -6459832 true "" "plot count turtles with [ immune2? ]"
